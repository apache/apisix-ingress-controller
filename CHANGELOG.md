...
* **Security fixes**: credentials are kept out of the logs, cross-namespace Consumer `secretRef` is gated by ReferenceGrant, and the Consumer webhook no longer reveals whether a Secret exists in another namespace.

---

## Upgrade Notes

**Kubernetes 1.31 or newer is recommended.** On an older cluster the controller logs a
warning and keeps running; nothing refuses to start. What needs 1.31 in practice are the
Gateway API 1.6.0 CRDs, whose CEL validation rules use features added in that release.
...
standard channel, which no longer serves `v1alpha2` and where the L4 reconcilers were
...
### TCPRoute and UDPRoute that pin a listener now match on `server_port`

[#2818](https://github.com/apache/apisix-ingress-controller/pull/2818). When a parentRef
sets `sectionName` or `port`, or the Gateway exposes more than one stream listener port,
the generated StreamRoute carries `server_port`. The Gateway listener port must then
equal the port APISIX actually listens on for streams, otherwise the connection no longer
matches the route:
...
Routes that do not pin a listener, on a Gateway with a single stream listener, keep the
previous behavior.
...
`ca.crt` key of a ConfigMap (Gateway API Core support) or a Secret (extension). Gateway
API 1.6 declares this at the Gateway level, not on the listener:
...
field of BackendTrafficPolicy and ApisixUpstream accepts `tcp`, `tls` and `udp` next to
the existing L7 values. They apply to stream routes only; setting one on an HTTP route

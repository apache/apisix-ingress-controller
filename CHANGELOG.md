...
**Breaking change**: the minimum supported Kubernetes version is now 1.31.0
(previously 1.26.0), as required by the Gateway API 1.6.0 CRDs. Install the
Gateway API CRDs with server-side apply
(`kubectl apply --server-side --force-conflicts`), because the v1.6.0 CRDs
exceed the annotation size limit of client-side apply.

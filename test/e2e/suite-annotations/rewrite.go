...
`, backendSvc, backendPort[0])
		...
		resp := s.NewAPISIXClient().GET("/sample/get").WithHeader("Host", "httpbin.org").WithHeader("X-Request-ID", "000").WithHeader("X-Test", "Test").Expect()
		resp.Status(http.StatusOK)
		resp.Body().Contains("\"X-Api-Version\": \"v1\"")
		resp.Body().Contains("\"X-Api-Engine\": \"Apisix\"")
		resp.Body().Contains("\"X-Request-ID\": \"123\"")

...
`
	...
	ginkgo.It("create a ApisixUpstreams with mTLS", func() {
		...
		assert.NoError(ginkgo.GinkgoT(), err, "create server cert secret error")
		err = s.NewClientCASecret(clientCASecret, rootCA, "")
		assert.NoError(ginkgo.GinkgoT(), err, "create client CA cert secret error")
		...
		assert.NoError(ginkgo.GinkgoT(), err, "create ApisixTls with client CA error")

		// check ssl in APISIX
		time.Sleep(10 * time.Second)
		apisixSsls, err := s.ListApisixSsl()
		assert.NoError(ginkgo.GinkgoT(), err, "list ssl error")
...
`, backendSvc, backendSvcPort[0])
		...
		assert.NoError(ginkgo.GinkgoT(), err, "generate cert")
...
`)
		...
		apisixRoutes, err = s.ListApisixRoutes()
		assert.NoError(ginkgo.GinkgoT(), err, "list routes error")
		assert.Len(ginkgo.GinkgoT(), apisixRoutes, 2, "route number not expect")

		s.NewAPISIXClient().GET("/ip").WithHeader("Host", "upstream-is-mtls.httpbin.local").Expect().Status(http.StatusOK).Body().Raw()
	})
})

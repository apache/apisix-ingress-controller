...
func initURL() (string, error) {
	cmd := exec.Command("sh", "testdata/wolf-rbac/ip.sh", "ip")
	ip, err := cmd.Output()
	if err != nil {
		return "", fmt.Errorf("Failed to get wolf-server IP, err is %s", err.Error())
	}
	if len(ip) == 0 {
		cmd = exec.Command("sh", "testdata/wolf-rbac/ip.sh", "info")
		debug, err := cmd.Output()
		if err != nil {
			return "", fmt.Errorf("Failed to get wolf-server IP is empty, err is %s", err.Error())
		}
		return "", fmt.Errorf("Failed to get wolf-server IP is empty, err is %s", string(debug))
	}

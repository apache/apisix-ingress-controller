...
func (c *apisixConsumerController) sync(ctx context.Context, ev *types.Event) error {
	...
	if ev.Type == types.EventDelete {
		...
		ac = ev.Tombstone.(*configv2.ApisixConsumer)
	}
...
func (c *apisixConsumerController) onUpdate(oldObj, newObj interface{}) {
	prev := oldObj.(*configv2.ApisixConsumer)
	curr := newObj.(*configv2.ApisixConsumer)
...
func (c *apisixConsumerController) onDelete(obj interface{}) {
	ac, ok := obj.(*configv2.ApisixConsumer)
	if !ok {
		tombstone, ok := obj.(cache.DeletedFinalStateUnknown)
		if !ok {
			return
		}
		ac = tombstone.Obj.(*configv2.ApisixConsumer)
	}

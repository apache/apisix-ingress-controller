...
type Endpoint interface {
	...
	BuildEpss(ns, name string, port int) []v1.Node
}
...
func (epr *EndpointRequest) BuildEpss(ns, name string, port int) []v1.Node {
	nodes := make([]v1.Node, 0)
	epInformers := kube.EndpointsInformer
	if ep, err := epInformers.Lister().Endpoints(ns).Get(name); err != nil {
		glog.Errorf("find endpoint %s/%s err: %s", ns, name, err.Error())
...
// BuildEpss build nodes from endpointSlices for upstream
func BuildEpss(ns, name string, port int) []v1.Node {
	nodes := make([]v1.Node, 0)
	epsInformers := kube.EndpointSliceInformer
	if eps, err := epsInformers.Lister().EndpointSlices(ns).Get(name); err != nil {
		glog.Errorf("find endpointSlice %s/%s err: %s", ns, name, err.Error())
	} else {

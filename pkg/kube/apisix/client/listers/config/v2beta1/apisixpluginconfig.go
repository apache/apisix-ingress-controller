...
type ApisixPluginConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta1.ApisixPluginConfig, err error)
...
// List lists all ApisixPluginConfigs in the indexer.
func (s *apisixPluginConfigLister) List(selector labels.Selector) (ret []*v2beta1.ApisixPluginConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta1.ApisixPluginConfig))
	})
	return ret, err
}
...
type ApisixPluginConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta1.ApisixPluginConfig, error)
...
// List lists all ApisixPluginConfigs in the indexer for a given namespace.
func (s apisixPluginConfigNamespaceLister) List(selector labels.Selector) (ret []*v2beta1.ApisixPluginConfig, err error) {
...
// Get retrieves the ApisixPluginConfig from the indexer for a given namespace and name.
func (s apisixPluginConfigNamespaceLister) Get(name string) (*v2beta1.ApisixPluginConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta1.Resource("apisixpluginconfig"), name)
	}
	return obj.(*v2beta1.ApisixPluginConfig), nil
}

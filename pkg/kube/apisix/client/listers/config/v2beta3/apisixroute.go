...
type ApisixRouteLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta3.ApisixRoute, err error)
...
// List lists all ApisixRoutes in the indexer.
func (s *apisixRouteLister) List(selector labels.Selector) (ret []*v2beta3.ApisixRoute, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta3.ApisixRoute))
	})
	return ret, err
}
...
type ApisixRouteNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta3.ApisixRoute, error)
...
// List lists all ApisixRoutes in the indexer for a given namespace.
func (s apisixRouteNamespaceLister) List(selector labels.Selector) (ret []*v2beta3.ApisixRoute, err error) {
...
// Get retrieves the ApisixRoute from the indexer for a given namespace and name.
func (s apisixRouteNamespaceLister) Get(name string) (*v2beta3.ApisixRoute, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta3.Resource("apisixroute"), name)
	}
	return obj.(*v2beta3.ApisixRoute), nil
}

...
type ApisixTlsLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta3.ApisixTls, err error)
...
// List lists all ApisixTlses in the indexer.
func (s *apisixTlsLister) List(selector labels.Selector) (ret []*v2beta3.ApisixTls, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta3.ApisixTls))
	})
	return ret, err
}
...
type ApisixTlsNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta3.ApisixTls, error)
...
// List lists all ApisixTlses in the indexer for a given namespace.
func (s apisixTlsNamespaceLister) List(selector labels.Selector) (ret []*v2beta3.ApisixTls, err error) {
...
// Get retrieves the ApisixTls from the indexer for a given namespace and name.
func (s apisixTlsNamespaceLister) Get(name string) (*v2beta3.ApisixTls, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta3.Resource("apisixtls"), name)
	}
	return obj.(*v2beta3.ApisixTls), nil
}

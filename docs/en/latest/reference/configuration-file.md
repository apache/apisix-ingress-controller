...
`listener_port_match_mode` turns a Gateway listener port into a `server_port` route variable, binding a route to a specific listener port. Two preconditions decide whether it takes effect.
...
With the default `off`, no `server_port` variable is emitted at all, so no route is pinned to a port. Keeping an HTTPS-only route off the plaintext port is a separate matter that does not depend on this setting; see [HTTPS listeners and plaintext requests](../concepts/gateway-api.md#https-listeners-and-plaintext-requests).

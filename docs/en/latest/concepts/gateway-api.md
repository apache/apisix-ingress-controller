...
| Fields                                               | Status               | Notes                                                                                          |
|------------------------------------------------------|----------------------|------------------------------------------------------------------------------------------------|
| `spec.listeners[].port`               | Partially supported | Controls `server_port` route-var injection; off by default and enabled via [`listener_port_match_mode`](../reference/configuration-file.md) (`auto` / `explicit`). The controller cannot dynamically open data plane ports, so APISIX must already listen on the specified port. See [Listener port matching](../reference/configuration-file.md#listener-port-matching) for the preconditions, including that a listener with a `hostname` is never pinned to a port. |

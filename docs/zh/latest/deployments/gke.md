---
title: 在 Google Cloud GKE 上安装 Ingress APISIX 
---
...
本文档介绍如何在 [Google Cloud GKE](https://cloud.google.com/kubernetes-engine)上安装 Ingress APISIX。

## 前提条件

* 在 GKE 创建一个Kubernetes Service
* 安装 [Google Cloud SDK](https://cloud.google.com/sdk) 并获取凭证或者只使用 [Cloud Shell](https://cloud.google.com/shell)
...
服务网关类型要设置成 `LoadBalancer` (详见 [GKE Load Balancer](https://cloud.google.com/kubernetes-engine/docs/concepts/service#services_of_type_loadbalancer) ), 这样客户机就可以通过负载访问 Apache APISIX 。您可以通过执行如下命令找到负载的 ip :

```shell
kubectl get service apisix-gateway --namespace ingress-apisix -o jsonpath='{.status.loadBalancer.ingress[].ip}'
```

另外一个值得注意的是 `allow.ipList` 字段应根据 [Pod CIRD configuration of GKE](https://cloud.google.com/kubernetes-engine/docs/how-to/flexible-pod-cidr) 规则进行定制， 以便 apisix-ingress-controller 实例可以访问 APISIX 实例（ 资源推送 ）。
